...
from recastgui.handlers import alignment
from recastgui.read_write import rec, regtext
...
class ProjectionHandler():
    ...
    def Update(self):
        ...
        if self.status == enums.ProjectionHandlerState.Saved:
        ...
        elif self.status == enums.ProjectionHandlerState.Registered:
            self.filelist = regtext.Read(self.reg)
            ...
            if self.tiltseries.nproj == len(self.filelist):
                self.status = enums.ProjectionHandlerState.Checked
        elif self.status == enums.ProjectionHandlerState.NewData:
            ...
            self.target.Update(self.tiltseries)
            self.status = enums.ProjectionHandlerState.Checked
        elif self.status ==  enums.ProjectionHandlerState.SaveRequested:
            rec.Write(self.save, self.tiltseries)
            self.target.Update(self.tiltseries)
            self.status = enums.ProjectionHandlerState.Checked
        elif self.status == enums.ProjectionHandlerState.ResetRequested:
